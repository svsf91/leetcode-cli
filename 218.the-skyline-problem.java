...
class Solution {
    ...
    public List<int[]> getSkyline(int[][] buildings) {
        PriorityQueue<Edge> pq = new PriorityQueue<>((a, b) -> a.pos - b.pos);
        TreeMap<Integer, Integer> height = new TreeMap<>();
        for (int[] building : buildings) {
            int from = building[0];
            int to = building[1];
            int h = building[2];
            pq.offer(new Edge(from, h, true));
            pq.offer(new Edge(to, h, false));
        }
        List<int[]> res = new ArrayList<>();
        int preHeight = 0;
        while (!pq.isEmpty()) {
            int pos = pq.peek().pos;
            while (!pq.isEmpty() && pq.peek().pos == pos) {
                Edge cur = pq.poll();
                if (cur.isStart) {
                    height.put(cur.height, height.getOrDefault(cur.height, 0) + 1);
                } else {
                    height.put(cur.height, height.get(cur.height) - 1);
                    if (height.get(cur.height) == 0) {
                        height.remove(cur.height);
                    }
                }
            }
            int h = height.isEmpty() ? 0 : height.lastKey();
            if (h != preHeight) {
                res.add(new int[] { pos, h });
                preHeight = h;
            }
        }
        return res;
